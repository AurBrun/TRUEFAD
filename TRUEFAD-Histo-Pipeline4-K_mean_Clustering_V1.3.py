...
def Cluster_image(init, df, Clusters):
    ...
    init += 1
    aT2B_array = df.iloc[:, init].values
    bT2B_array = aT2B_array[~pd.isna(aT2B_array)]
    T2B_array = bT2B_array[bT2B_array != '']
    init += 1
    #Trim the arrays to be even in case there is corruption of the native TRUEFAD excel file export, then concatenate them
    min_length = min(len(T1_array), len(T2A_array), len(T2B_array))
    T1 = T1_array[:min_length]
    T2A = T2A_array[:min_length]
    T2B = T2B_array[:min_length]
    data = np.column_stack((T1, T2A, T2B))
...
def read_excel():
    ...
    if filepath:
        try:
            ...
            num_blocks = int((df.shape[1] + 1)/10)
            init = 6 #Main indexation parameter
            Metrics = []
            for block in range(num_blocks):                              
                cluster_labels, Metrics_block = Cluster_image(init, df, Clusters) #Run clusterization on T1, T2A, T2B signal for this block
                ...
                Count = df.iloc[:, init-9].values
                Area = df.iloc[:, init-8].values
                Perim = df.iloc[:, init-7].values
                Feret = df.iloc[:, init-6].values
                Circ = df.iloc[:, init-5].values
                Round = df.iloc[:, init-4].values
                T1 = df.iloc[:, init-3].values
                T2 = df.iloc[:, init-2].values
                T3 = df.iloc[:, init-1].values
                Label = df.iloc[:, init].values
                min_length = min(len(Count), len(Area), len(Perim), len(Feret), len(Circ), len(Round), len(T1), len(T2), len(T3), len(Label))
